...
- RepoRoot/WorkspacePath can auto-resolve when omitted; module paths auto-detect from bundle external/*.
- Script auto-attempts `git submodule update --init --recursive` when `external/*` modules are missing (can be disabled).
...
function Try-Initialize-Submodules(
  [string]$BundleRootPath,
  [string[]]$ExpectedModulePaths,
  [bool]$Disabled
) {
  if ($Disabled) {
    Write-Info "SkipSubmoduleInit is set; skip submodule auto-init."
    return
  }
  $missing = $false
  foreach ($p in $ExpectedModulePaths) {
  ...
  if ($null -eq $gitCmd) {
    Write-Warn "git not found; skip submodule auto-init."
    return
  }
  if (-not (Test-Path -LiteralPath (Join-Path $BundleRootPath ".git"))) {
    Write-Info "Bundle root is not a git repository; skip submodule auto-init."
    return
  }
  Write-Step "External modules missing; attempting submodule init/update"
  $prev = Get-Location
  try {
    Set-Location -LiteralPath $BundleRootPath
    & git submodule update --init --recursive
    if ($LASTEXITCODE -ne 0) {
      Write-Warn "Submodule init failed (exit code: $LASTEXITCODE). Continue with existing local files."
      return
    }
    Write-Ok "Submodule init completed."
  }
  catch {
    Write-Warn "Submodule init error: $($_.Exception.Message)"
  }
  finally {
...
$scriptBundleRoot = [System.IO.Path]::GetFullPath((Join-Path $PSScriptRoot ".."))

if ([string]::IsNullOrWhiteSpace($RepoRoot)) {
  $RepoRoot = Resolve-FirstExistingPath @(
    (Join-Path $scriptBundleRoot "common\claude-code-main"),
    (Join-Path $scriptBundleRoot "claude-code-main")
  )
  if (-not [string]::IsNullOrWhiteSpace($RepoRoot)) {
    Write-Info "Auto-detected RepoRoot: $RepoRoot"
  }
}
...
if ([string]::IsNullOrWhiteSpace($WorkspacePath)) {
  ...
  elseif (-not [string]::IsNullOrWhiteSpace($env:CODEX_WORKSPACE)) {
    ...
    if (Test-Path -LiteralPath (Join-Path $cwd ".git")) {
      ...
      $WorkspacePath = Join-Path $env:USERPROFILE "Documents\codex-workspace"
    }
  }
  Write-Info "Auto-resolved WorkspacePath: $WorkspacePath"
}
...
Try-Initialize-Submodules -BundleRootPath $bundleRoot -ExpectedModulePaths @(
  (Join-Path $bundleRoot "external\modules\mod-a"),
  (Join-Path $bundleRoot "external\modules\mod-b"),
  (Join-Path $bundleRoot "external\modules\mod-c"),
  (Join-Path $bundleRoot "external\modules\mod-d")
) -Disabled $SkipSubmoduleInit.IsPresent

if ([string]::IsNullOrWhiteSpace($SuperpowersSourcePath)) {
  $SuperpowersSourcePath = Resolve-FirstExistingPath @(
    (Join-Path $bundleRoot "external\modules\mod-a"),
    (Join-Path $bundleRoot "mod-a")
  )
  if (-not [string]::IsNullOrWhiteSpace($SuperpowersSourcePath)) {
    Write-Info "Auto-enabled superpowers source: $SuperpowersSourcePath"
  }
}
if ([string]::IsNullOrWhiteSpace($OpenSpaceSourcePath)) {
  $OpenSpaceSourcePath = Resolve-FirstExistingPath @(
    (Join-Path $bundleRoot "external\modules\mod-b"),
    (Join-Path $bundleRoot "mod-b")
  )
  if (-not [string]::IsNullOrWhiteSpace($OpenSpaceSourcePath)) {
    Write-Info "Auto-enabled OpenSpace source: $OpenSpaceSourcePath"
  }
}
if ([string]::IsNullOrWhiteSpace($SelfImprovingSourcePath)) {
  $SelfImprovingSourcePath = Resolve-FirstExistingPath @(
    (Join-Path $bundleRoot "external\modules\mod-c"),
    (Join-Path $bundleRoot "mod-c")
  )
  if (-not [string]::IsNullOrWhiteSpace($SelfImprovingSourcePath)) {
    Write-Info "Auto-enabled self-improving source: $SelfImprovingSourcePath"
  }
}
if ([string]::IsNullOrWhiteSpace($ProactiveSourcePath)) {
  $ProactiveSourcePath = Resolve-FirstExistingPath @(
    (Join-Path $bundleRoot "external\modules\mod-d"),
    (Join-Path $bundleRoot "mod-d")
  )
  if (-not [string]::IsNullOrWhiteSpace($ProactiveSourcePath)) {
    Write-Info "Auto-enabled proactive source: $ProactiveSourcePath"
  }
}
...
if (-not (Test-Path -LiteralPath $WorkspacePath)) {
  Write-Info "WorkspacePath not found; creating: $WorkspacePath"
  Ensure-Directory -PathValue $WorkspacePath
}
...
if ($DisableGitReview) {
  $gitReviewEnabled = $false
}
elseif ($EnableGitReview) {
  $gitReviewEnabled = $true
}
Write-Info ("Git review automation: " + ($(if ($gitReviewEnabled) { "enabled" } else { "disabled" })))

$nightlyMemoryEnabled = $true
if ($DisableNightlyMemory) {
  $nightlyMemoryEnabled = $false
}
elseif ($EnableNightlyMemory) {
  $nightlyMemoryEnabled = $true
}
Write-Info ("Nightly memory automation: " + ($(if ($nightlyMemoryEnabled) { "enabled" } else { "disabled" })))
...
if ($nightlyMemoryEnabled) {
  ...
  Write-Info "Nightly memory automation disabled by flag; task not created."
}
